...
$sql = "SELECT * from invoices where accountid = " . $id;
$sql .= " AND (";
$sql .= " 	 generate_date  like '%" . strtolower($search) . "%' ";
$sql .= " 	or number  like '%" . strtolower($search) . "%' ";
...
echo "	<tr>\n";
echo "		<td width='50%' align='left' valign='top' nowrap='nowrap'><b>Invoices (" . $num_rows . ")</b></td>\n";
...
while ($row = mysqli_fetch_array($result)) {
    ...
    echo $row['prefix'] . "" . $row['number'];
    echo "	</td>\n";

    echo "	<td valign='top' class='" . $row_style[$c] . "'>";
    if ($row['generate_type'] == 0) {
        echo "Automatically";
    } else {
        echo "Manually";
    }
    ...
    echo "	</td>";
    echo "	<td valign='top' class='" . $row_style[$c] . "'>" . $row['generate_date'] . "</td>\n";
    echo "	<td valign='top' class='" . $row_style[$c] . "'>" . $row['from_date'] . "</td>\n";
    echo "	<td valign='top' class='" . $row_style[$c] . "'>" . $row['due_date'] . "</td>\n";
    echo "	<td valign='top' class='" . $row_style[$c] . "'>" . $row['to_date'] . "</td>\n";

    $sql1 = "SELECT * from invoice_details where invoiceid = '" . $row['id'] . "'";
    $result1 = mysqli_query($conn, $sql1);
    while ($amount = mysqli_fetch_array($result1)) {
        echo "	<td valign='top' class='" . $row_style[$c] . "'>" . $amount['debit'] . "</td>\n";
    }

    echo "    <td class='list_control_icons " . $row_style[$c] . "'>";
